...
mod linux_tests {
    ...
    fn test_execve_event_captured_on_process_spawn() {
        ...
        assert!(!execve_events.is_empty(), "Should capture at least one execve event");
    }
    ...
    fn test_execve_event_contains_filename() {
        ...
        let has_comm = execve_events.iter().any(|e| {
            e.comm.as_ref().map(|c| !c.is_empty()).unwrap_or(false)
        });
    ...
    fn test_execve_event_timestamp() {
        ...
        for event in execve_events {
            assert!(event.timestamp >= before, "Event timestamp should be after test start");
        }
    }
}
