...
impl SniffConfig {
    ...
    pub fn from_env_and_args(
        ...
        let ai_provider_str = ai_provider_arg.map(|s| s.to_string()).unwrap_or_else(|| {
        ...
        let output_dir = if output != "./stackdog-logs/" {
            ...
            PathBuf::from(
                env::var("STACKDOG_SNIFF_OUTPUT_DIR").unwrap_or_else(|_| output.to_string()),
            )
        };
...
mod tests {
    ...
    fn test_sniff_config_env_sources_merged_with_cli() {
        ...
        let config = SniffConfig::from_env_and_args(
            ...
            false,
            "./stackdog-logs/",
            Some("/var/log/app.log,/var/log/syslog"),
            30,
            None,
            None,
            None,
